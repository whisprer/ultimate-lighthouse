...
#include <jsonifier/Index.hpp>  // Master include for everything
...
private:
    ...
    // RTC-Jsonifier high-performance serializer with custom allocator
    jsonifier::jsonifier_core<> serializer_;
    
    // Pre-allocated SIMD-aligned buffers for zero-allocation hot path
    alignas(64) char json_buffer_[8192];
    alignas(64) char network_buffer_[8192];
...
private:
    ...
    void send_beacon() {
        ...
        // Zero-allocation JSON serialization using RTC-Jsonifier SIMD engine
        jsonifier::string json_output;
        json_output.reserve(1024); // Pre-allocate with RTC-Jsonifier's custom allocator
        ...
        if (serializer_.serializeJson(msg, json_output)) {
            ...
            std::cerr << "SIMD JSON serialization failed for beacon #" << msg.sequence_number << std::endl;
        }
    }
};
...
private:
    ...
    void handle_client(int client_fd, const sockaddr_in& client_addr) {
        ...
        std::cout << "Client connected: " << client_ip << ":" << ntohs(client_addr.sin_port) << std::endl;
            ...
            // Use SIMD-aligned buffer for maximum throughput
            alignas(64) char buffer[16384];  // SIMD-friendly size
    ...
    void process_message(const char* data, size_t length, const char* client_ip) {
        try {
            ...
            // Use optimized parsing with minified JSON option for maximum speed
            constexpr jsonifier::parse_options opts{ .minified = true };
            if (parser_.parseJson<opts>(msg, json_data)) {
                std::cout << "[" << client_ip << "] SIMD-parsed Beacon #" << msg.sequence_number 
                         << " from " << msg.source_id 
                ...
                auto errors = parser_.getErrors();
                if (!errors.empty()) {
                    for (const auto& error : errors) {
                        std::cerr << "Parse error: " << error.reportError() << std::endl;
                    }
                }
