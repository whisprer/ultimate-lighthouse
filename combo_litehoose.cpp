...
// Platform-specific includes
    ...
    // TCP_NODELAY is in winsock2.h but sometimes needs explicit definition
    #ifndef TCP_NODELAY
...
// Error helper for cross-platform socket errors
inline std::string get_socket_error() {
        ...
        return std::string("Socket error ") + std::to_string(error) + ": " + msgbuf;
    #else
        ...
        if (serializer_.serializeJson<opts>(msg, json_output)) {
            ...
            auto serialize_us = std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                int bytes_sent = sendto(socket_fd_, 
                                      json_output.data(), 
                                      static_cast<int>(json_output.size()), 
                                      0,
            ...
            if (bytes_received > 0) {
                ...
                break; // Client disconnected
                    ...
                    if (error != WSAEWOULDBLOCK) {
                        std::cerr << "Receive failed: " << get_socket_error() << std::endl;
                        break;
                    }
                #else
